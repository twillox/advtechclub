...
export default function Admin() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (form.image && !form.image.match(/\.(jpeg|jpg|png|webp|gif)(\?.*)?$/i)) {
    ...
    try {
      ...
      setCreating(false);
      setForm({ title: "", description: "", date: "", xp: 10, category: "workshop", image: "" });
      fetchData();
      alert("Event Created!");
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
                  ...
                  {creating && (
                    <form onSubmit={handleCreate} className="space-y-5 animate-in fade-in zoom-in duration-700 pt-6">
                      <div className="space-y-4">
                        ...
                        />
                        <input
                          type="text" placeholder="Paste image URL (jpg/png)"
                          className="w-full bg-surface/10 border border-surface/10 rounded-[20px] py-6 px-8 text-xs font-bold focus:ring-2 focus:ring-primary/40 focus:outline-none transition-all text-white placeholder:text-surface/30 shadow-inner"
      ...
      {/* Data Export Sync Warning */}
      {dataWarning && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 bg-amber-500 text-white px-8 py-5 rounded-[32px] shadow-2xl z-[1000] flex items-center gap-6 animate-in slide-in-from-bottom border-4 border-white">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-widest">{dataWarning.warningMessage}</p>
            <p className="text-[10px] font-bold mt-1 opacity-80 italic">Auto-Purge Scheduled for: {new Date(dataWarning.deletionDate).toLocaleDateString()}</p>
          </div>
          <a
            href={`${API_BASE_URL}${dataWarning.downloadUrl}`}
            download
            target="_blank"
            rel="noreferrer"
            onClick={() => setDataWarning(null)}
            className="bg-black hover:bg-white hover:text-black py-4 px-6 rounded-2xl text-[10px] uppercase font-black tracking-widest transition-all whitespace-nowrap cursor-pointer shadow-inner"
          >
            Secure Copy
          </a>
          <button onClick={() => setDataWarning(null)} className="text-white/50 hover:text-white transition-all cursor-pointer">
